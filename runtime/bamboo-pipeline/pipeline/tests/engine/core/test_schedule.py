...
class ScheduleTestCase(TestCase):
    ...
    @mock.patch(PIPELINE_STATUS_FILTER, mock.MagicMock(return_value=MockQuerySet(exists_return=False)))
    @mock.patch(SIGNAL_VALVE_SEND, mock.MagicMock())
    def test_schedule__status_is_not_running(self):
        mock_ss = MockScheduleService(
    ...
    @mock.patch(PIPELINE_STATUS_FILTER, mock.MagicMock(return_value=MockQuerySet(exists_return=True)))
    @mock.patch(PIPELINE_SCHEDULE_SERVICE_FILTER, mock.MagicMock(return_value=MockQuerySet(update_return=0)))
    @mock.patch(SIGNAL_VALVE_SEND, mock.MagicMock())
    def test_schedule__get_lock_failed_and_multi_calback_not_enbaled(self):
        mock_ss = MockScheduleService(
            schedule_return=True,
            wait_callback=False,
            schedule_done=False,
